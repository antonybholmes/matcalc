...
public class ColumnGroupTreePanel extends ModernComponent {
   ...
   */
  private ModernClickWidget mAddButton = new ModernButton(AssetService.getInstance().loadIcon(PlusVectorIcon.class, 16))
   ...
   */
  private ModernClickWidget mRemoveButton = new ModernButton(AssetService.getInstance().loadIcon("trash_bw", 16))
      .setButtonStyle(ButtonStyle.CIRCLE);
  ...
  private class TreeEvents implements TreeEventListener {
     ...
     * (non-Javadoc)
     * 
     * @see org.abh.lib.ui.modern.tree.TreeEventListener#treeNodeDoubleClicked(org.
  ...
  protected void createTree(Collection<XYSeries> groups, boolean clear) {
    ...
    for (XYSeries group : groups) {
       ...
       * 
       * 
       * for (int i : XYSeries.findColumnIndices(mMatrix, group)) { TreeNode<XYSeries>
       * childNode = new TreeNode<XYSeries>( mMatrix.getColumnName(i));
       * 
       * // Don't render the children, the parent will. This is // because we allow
       * dragging and we don't want users to // drag the children around
       * childNode.setVisible(false);
       * 
       * groupNode.addChild(childNode); }
       * 
       * // Cannot inherit children groupNode.setExpanded(false);
       * 
  ...
  private void importGroups() throws IOException, InvalidFormatException {
    ...
    DataFrame groupsMatrix = Excel.convertToMatrix(file, 1, TextUtils.emptyList(), 0, TextUtils.TAB_DELIMITER);
