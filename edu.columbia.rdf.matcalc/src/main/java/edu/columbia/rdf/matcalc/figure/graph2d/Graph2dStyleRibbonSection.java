...
public class Graph2dStyleRibbonSection extends RibbonSection implements ModernClickListener {
	...
	/*
	private ModernCheckButton mJoinedFilledButton = 
			new RibbonLargeRadioButton2("Joined Filled", 
					new Raster24Icon(new JoinedFilledStyleIcon()),
					"Style Joined Filled", 
					"Peaks are drawn as a continuous filled line.");
	
	private ModernCheckButton mJoinedFilledTransButton = 
			new RibbonLargeRadioButton2("Joined Filled Transparent", 
					new Raster24Icon(new JoinedFilledTransStyleIcon()),
					"Style Joined Filled Transparent", 
					"Peaks are drawn as a continuous filled line.");
	
	private ModernCheckButton mJoinedButton = 
			new RibbonLargeRadioButton2("Joined", 
					new Raster24Icon(new JoinedStyleIcon()),
					"Style Joined", 
					"Peaks are drawn as a continuous line.");
	
	private ModernCheckButton mLinesButton = 
			new RibbonLargeRadioButton2("Line", 
					new Raster24Icon(new LinesStyleIcon()),
					"Style Line", 
					"Peaks are drawn as lines.");
	
	private ModernCheckButton mBarsButton = 
			new RibbonLargeRadioButton2("Bars", 
					new Raster24Icon(new BarsStyleIcon()),
					"Style Bars", 
	...
	*/
	...
	public Graph2dStyleRibbonSection(Ribbon ribbon,
		...
		group.add(mJoinedFilledButton);
		group.add(mJoinedFilledTransButton);
		group.add(mJoinedButton);
		group.add(mLinesButton);
		...
		mLinesButton.addClickListener(this);
	...
	private void change(ModernClickEvent e) {
		...
		if (mJoinedFilledButton.isSelected()) {
			mStyleModel.set(PeakStyle.FILLED);
		} else if (mJoinedButton.isSelected()) {
			mStyleModel.set(PeakStyle.JOINED);
		} else if (mLinesButton.isSelected()) {
			mStyleModel.set(PeakStyle.LINES);
		} else if (mBarsButton.isSelected()) {
			mStyleModel.set(PeakStyle.BARS);
		} else {
