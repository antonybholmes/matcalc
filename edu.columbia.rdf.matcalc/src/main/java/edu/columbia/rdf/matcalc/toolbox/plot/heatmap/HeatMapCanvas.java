...
public class HeatMapCanvas extends PlotBoxRow {
   ...
   * The horizontal gap.
   */
  private Dimension horizontalGap = new Dimension(MatrixPlotElement.BLOCK_SIZE,
      0);

  /**
   * The vertical gap.
   */
  private Dimension verticalGap = new Dimension(0,
      MatrixPlotElement.BLOCK_SIZE);
  ...
  public HeatMapCanvas(DataFrame matrix, XYSeriesGroup groups,
    ...
    PlotBox rowLabelsSpaceBox = new PlotBoxEmpty(
        rowLabelsBox.getPreferredSize().height, 0);
    ...
    if (groupProperties.show && groups.getCount() > 0) {
      ...
      element = new GroupLabelPlotElement(matrix, aspectRatio, groups,
          groupProperties);
