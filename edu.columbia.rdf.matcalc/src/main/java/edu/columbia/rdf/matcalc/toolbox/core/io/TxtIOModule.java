...
public class TxtIOModule extends IOModule {
  ...
  private static final GuiFileExtFilter TXT_FILTER = FileFilterService.getInstance().getFilter("txt");
  ...
  public DataFrame open(final MainMatCalcWindow window, final Path file, FileType type, int headers, int rowAnnotations,
      String delimiter, Collection<String> skipMatches) throws IOException {

    rowAnnotations = OpenFiles.estimateRowAnnotations(file, headers, skipMatches);

    delimiter = OpenFiles.guessDelimiter(file, skipMatches);

    boolean numerical = OpenFiles.guessNumerical(file, headers, delimiter, rowAnnotations, skipMatches);

    ImportDialog dialog = new ImportDialog(window, headers, rowAnnotations, false, delimiter, numerical);
    ...
    return read(window, file, dialog.isNumerical() ? FileType.NUMERICAL : FileType.MIXED, dialog.getHasHeader() ? 1 : 0,
        dialog.getIndexCols(), dialog.getDelimiter(), dialog.getSkipLines());
  }
  ...
  public DataFrame read(final MainMatCalcWindow window, final Path file, FileType type, int headers, int rowAnnotations,
    ...
    if (delimiter.equals(",")) {
      ...
      if (type == FileType.NUMERICAL) {
        ...
        return new MixedMatrixParser(headers, skipLines, rowAnnotations, delimiter).parse(file);
