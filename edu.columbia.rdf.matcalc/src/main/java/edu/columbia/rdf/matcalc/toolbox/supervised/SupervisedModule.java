 ...
 */
public class SupervisedModule extends Module implements ModernClickListener {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(SupervisedModule.class);

  private static final int MAX_ROW_COUNT = SettingsService.getInstance()
      .getInt("matcalc.modules.supervised.max-plot-rows");
  ...
  public void run(ArgParser ap) throws IOException {
    ...
    DataFrame m = new DoubleMatrixParser(1, TextUtils.EMPTY_LIST, 1, TextUtils.TAB_DELIMITER).parse(file);

    List<XYSeries> groups = XYSeries.loadJson(groupFile);

    if (mode.equals("ttest")) {
      cliTTest(name, m, groups, alpha, classificationAlpha, topGenes, minFold, minZ, posZ, negZ, test, fdrType,
          isLog2Data, prefix, dir);
    }
  }

  private static void cliTTest(String name, DataFrame m, List<XYSeries> groups, double alpha,
      double classificationAlpha, int topGenes, double minFold, double minZ, boolean posZ, boolean negZ, TestType test,
      FDRType fdrType, boolean isLog2Data, String prefix, Path dir) throws IOException {
    for (int i = 0; i < groups.size(); ++i) {
      ...
      for (int j = i + 1; j < groups.size(); ++j) {
        ...
        Path out = dir.resolve(
            name + "_" + prefix + "_" + g1.getName().toLowerCase() + "_" + g2.getName().toLowerCase() + "_ttest.txt");

        LOG.info("Writing {}...", out);

        DataFrame ret = ttest(m, alpha, classificationAlpha, minFold, minZ, posZ, negZ, topGenes, test, fdrType, g1, g2,
            isLog2Data);
    ...
    default:
      ...
      case ONE_VS_REST:
        statTestOneVsRest(collapsedM, alpha, classificationAlpha, minFold, minZ, posZ, negZ, topGenes, testType,
            fdrType, sortType, groups, rowGroups, isLog2, log2Data);
        break;
      case PAIRWISE:
        statTestPairwise(collapsedM, alpha, classificationAlpha, minFold, minZ, posZ, negZ, topGenes, testType, fdrType,
            sortType, groups, rowGroups, isLog2, log2Data);
        break;
      default:
        // One group vs another
        statTest(collapsedM, alpha, classificationAlpha, minFold, minZ, posZ, negZ, topGenes, testType, fdrType,
            sortType, g1, g2, groups, rowGroups, isLog2, log2Data, plotType, keepHistory);
        break;
      }
    }
  }
  ...
  public void statTestOneVsRest(DataFrame m, double alpha, double classificationAlpha, double minFold, double minZ,
      boolean posZ, boolean negZ, int topGenes, TestType test, FDRType fdrType, SortType sortType,
      XYSeriesModel allSeries, XYSeriesModel rowSeries, boolean isLog2Data, boolean log2Data) {
    ...
    for (XYSeries s : allSeries) {
      ...
      statTest(m, alpha, classificationAlpha, minFold, minZ, posZ, negZ, topGenes, test, fdrType, sortType, s, s2,
  ...
  public void statTest(DataFrame m, double alpha, double classificationAlpha, double minFold, double minZ, boolean posZ,
      boolean negZ, int topGenes, TestType test, FDRType fdrType, SortType sortType, XYSeries g1, XYSeries g2,
      XYSeriesModel groups, XYSeriesModel rowGroups, boolean isLog2Data, boolean log2Data, PlotType plotType,
      boolean keepHistory) {
    ...
    meansM.getIndex().setAnnotation(g1.getName() + " mean", DoubleMatrix.means(foldChangesM, g1));
    meansM.getIndex().setAnnotation(g2.getName() + " mean", DoubleMatrix.means(foldChangesM, g2));
    ...
    double[] fdr = Statistics.fdr(foldFilterM.getIndex().getValues("P-value"), fdrType);
    ...
    String comparison = g1.getName() + "_vs_" + g2.getName() + " (p <= " + classificationAlpha + ")";
    ...
    if (posZ) {
      posScores = CollectionUtils
          .reverseSort(CollectionUtils.subList(scoresIndexed, MathUtils.ge(scoresIndexed, minZ)));
    } else {
    ...
    if (negZ) {
      negScores = CollectionUtils.sort(CollectionUtils.subList(scoresIndexed, MathUtils.lt(scoresIndexed, -minZ)));
    } else {
    ...
    if (plotType != PlotType.NONE) {
      ...
      if (mNormalized.getRows() > MAX_ROW_COUNT) {
        ...
        ModernDialogStatus status = ModernMessageDialog.createDialog(mParent, MessageDialogType.WARNING_OK_CANCEL,
      ...
      if (plot) {
        ClusterProps p = new ClusterProps();

        LegacyHeatMapModule.scaleLargeMatrixImage(m, p);

        mParent.history().addToHistory(new HeatMapMatrixTransform(mParent, mNormalized, groups, comparisonGroups,
            rowGroups, countGroups, mParent.getTransformationHistory(), p));
      }
    }
  ...
  public static DataFrame ttest(DataFrame m, double alpha, double classificationAlpha, double minFold, double minZ,
      boolean posZ, boolean negZ, int topGenes, TestType test, FDRType fdrType, XYSeries g1, XYSeries g2,
      boolean isLog2Data) {
  ...
  public static double[] getP(DataFrame m, XYSeries g1, XYSeries g2, TestType test) {
    double[] pValues;
