...
public class ColorStandardizationRibbonSection extends RibbonSection
   ...
   */
  public ColorStandardizationRibbonSection(ModernWindow parent, Ribbon ribbon,
      ColorNormalizationModel model) {
    ...
    mMatrixButton.setSelected(
        model.get().getType() == ColorNormalizationType.ZSCORE_MATRIX);
    ...
    mRowButton.setSelected(
        model.get().getType() == ColorNormalizationType.ZSCORE_ROW);
    ...
    mColumnButton.setSelected(
        model.get().getType() == ColorNormalizationType.ZSCORE_COLUMN);
    ...
    mNormButton
        .setSelected(model.get().getType() == ColorNormalizationType.NORMALIZE);
    ...
    mNoneButton
        .setSelected(model.get().getType() == ColorNormalizationType.NONE);
    ...
    if (e.getSource().equals(mMatrixButton)) {
      ...
      if (dialog.getStatus() == ModernDialogStatus.OK) {
        if (dialog.getAuto()) {
          ...
          mModel.set(new ColorNormalization(ColorNormalizationType.NORMALIZE,
              dialog.getMin(), dialog.getMax()));
        }
      }
    } else {
