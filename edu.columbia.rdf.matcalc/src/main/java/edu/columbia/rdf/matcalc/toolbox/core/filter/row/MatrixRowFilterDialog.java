...
public class MatrixRowFilterDialog extends ModernDialogHelpWindow implements ModernClickListener {
	 ...
	 */
	private ModernTwoStateWidget mCheckExact = 
			new ModernCheckSwitch("Match entire cell contents", false);

	/**
	 * The member check in list.
	 */
	private ModernTwoStateWidget mCheckInList = 
			new ModernCheckSwitch("Find in list", true);

	/** The m check missing. */
	private ModernTwoStateWidget mCheckMissing = 
			new ModernCheckSwitch("Include missing entries");

	/** The m check case. */
	private ModernTwoStateWidget mCheckCase = 
	...
	//		new ModernDialogFlatButton(Ui.MENU_REMOVE,
	...
	private void setup() {
		...
		panel.add(new ModernLabel("Column"));
	...
	private void importIds() throws IOException, InvalidFormatException {
		...
		Path file = FileDialog
				.open(mParent)
				.filter(new XlsxTxtGuiFileFilter())
