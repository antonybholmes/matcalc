...
public class OpenFiles {
    ...
    for (Path file : files) {
      ...
      if (mWindow.mFilesModel.size() > 0 && mOpenMode == OpenMode.NEW_WINDOW) {
        ...
        for (IOModule module : modules) {
          DataFrame m = module.open(mWindow, file, mFileType, mHeaders, mIndexes, mDelimiter, mSkipLines);
  ...
  public boolean read(Collection<Path> files) throws IOException {
    ...
    for (Path file : files) {
      if (mWindow.mFilesModel.size() > 0 && mOpenMode == OpenMode.NEW_WINDOW) {
        ...
        for (IOModule module : modules) {
          DataFrame m = module.read(mWindow, file, mFileType, mHeaders, mIndexes, mDelimiter, mSkipLines);
  ...
  /**
   * Guess if matrix contains only numbers since we get a speed up for using
   * numerical matrices.
   * 
   * @param file
   * @return
