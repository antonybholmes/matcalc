 ...
 */
public class XYSeriesImportDialog extends ModernDialogTaskWindow implements ModernClickListener {
  ...
  private ModernButton mClearFilterButton = new RibbonButton("Clear Filter",
  ...
  private void refresh() {
    ...
    for (String name : mNames) {
       ...
       * 
       * ModernCheckBox checkUse = new ModernCheckBox(name, true,
  ...
  public List<XYSeries> getGroups() {
     ...
     * ret.add(series);
     * 
     * // Add group covering everything this group does not so we // create one
     * group per line
     * 
     * if (mCheckComplementaryGroup.isSelected()) { XYSeries s2 = new
     * XYSeries("Non " + name, Color.WHITE);
     * 
     * Set<Integer> columns =
     * CollectionUtils.toSet(XYSeries.findColumnIndices(mMatrix, series));
     * 
     * for (int i = 0; i < mMatrix.getColumnCount(); ++i) { if
     * (!columns.contains(i)) {
